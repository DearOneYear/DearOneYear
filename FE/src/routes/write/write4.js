...
function Write4() {
  ...
  return (
    <>
      {who === "tome" ? (
        <h2>나에게 편지 쓰는 중...(2/4)</h2>
      ) : (
        <h2>너에게 편지쓰는 중...(3/5)</h2>
      )}
      <h3>지금, 당신은 어떤 기분인가요?</h3>
      <button onClick={ClickEmotion}>행복을 나누고 싶은</button>
      <button onClick={ClickEmotion}>집에 가고 싶은</button>
      <br></br>
      <button onClick={ClickEmotion}>콧노래가 절로 나오는</button>
      <button onClick={ClickEmotion}>위로가 필요한</button>
      <br></br>
      <button onClick={ClickEmotion}>치킨이 땡기는</button>
      <button onClick={ClickEmotion}>아무것도 하기 싫은</button>
      <br></br>
